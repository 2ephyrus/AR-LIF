...
class CIFAR10DVS(EventsFramesDatasetBase):
    ...
    def download_and_extract(download_root: str, extract_root: str):
        for key in resource.keys():
            file_name = os.path.join(os.path.join(download_root, "download"), key + '.zip')
    ...
    def __init__(self, root: str, train: bool, split_ratio=0.9, use_frame=True, frames_num=10, split_by='number',
                 normalization='max', transform=None):
        '''
...
class DVSCifar10(Dataset):
    def __init__(self, root, train=True, transform=None, target_transform=None):
        ...
        self.resize = transforms.Resize(size=(48, 48),
                                        interpolation=torchvision.transforms.InterpolationMode.NEAREST)  # 48 48
        self.rotate = transforms.RandomRotation(degrees=30)
        self.shearx = transforms.RandomAffine(degrees=0, shear=(-30, 30))
    ...
    def __getitem__(self, index):
        ...
        for t in range(data.shape[0]):
            data_t = self.imgx(data[t, ...])
            data_t = self.resize(data_t)
            data_t = self.tensorx(data_t)
            # print(d_t.shape)
    ...
    def __len__(self):
        num = len(os.listdir(self.root))
        # print("data number is: ", num)
        return num
