...
def main():
    ...
    if args.dataset == 'cifar10' or args.dataset == 'cifar100':
        ...
        if args.cutupmix_auto:
            ...
            train_set = dataloader(root=args.data_dir, train=True, transform=transform_train, download=False)
            test_set = dataloader(root=args.data_dir, train=False, transform=transform_test, download=False)

            train_data_loader = torch.utils.data.DataLoader(
                dataset=train_set, batch_size=args.b, collate_fn=collate_fn,
                shuffle=True, drop_last=True, num_workers=args.j, pin_memory=True)

            test_data_loader = torch.utils.data.DataLoader(
                dataset=test_set, batch_size=args.b, shuffle=False,
        ...
        transoform_list = [transforms.RandomCrop(64), transforms.RandomHorizontalFlip(0.5)]
        if args.cutupmix_auto:
            transoform_list.append(autoaugment.TrivialAugmentWide())
        ...
        train_data_loader = torch.utils.data.DataLoader(
            train_data, batch_size=args.b, shuffle=True, num_workers=args.j, pin_memory=True)

        test_data_loader = torch.utils.data.DataLoader(
            test_data, batch_size=args.b, shuffle=False, num_workers=args.j, pin_memory=True)
    else:
    ...
    elif args.model in ['vggsnn', 'snn5_noAP']:
    ...
    def create_hook_fn(layer_name):
        def hook_fn(module, input, output):
            if not isinstance(output, torch.Tensor):
                print(f"Warning: Layer {layer_name} output is not Tensor，Type is {type(output)}")
                return
    ...
    for name, module in net.named_modules():
        if isinstance(module, (neuron.XLIF, neuron.softLIF, neuron.hardLIF)):
    ...
    for layer_name in layer_total_spikes:
        ...
        print(f"LAYER {layer_name}: {avg_firing_rate:.6f}")
